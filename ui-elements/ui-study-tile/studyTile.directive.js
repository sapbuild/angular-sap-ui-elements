        ...
        link: function (scope, element) {
            ...
            if (!existingPopup) {
                var popup = '<ui-popup id="study-url-popup-' + scope.idx + '" ' +
                    'class="study-url-popup" placement="bottom" on-fully-open="selectLink()">' +
                    '<label>Participant link</label><br>' +
                    '<ui-input ng-readonly="true" value="{{url}}" id="study-url-input-' + scope.idx + '" click-highlight></ui-input>' +
                    '</ui-popup>';
            ...
            scope.$on('$destroy', function () {
                var pp = document.getElementById('study-url-popup-' + scope.idx);
                angular.element(pp).remove();
                element.remove();
