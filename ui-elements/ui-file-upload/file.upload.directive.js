 ...
 * @param {boolean} multiple Indicates if the file upload can allow multiple files or is restricted to one.  The default
 * value is true.
 *
 * @example
...
module.exports = function ($compile, $upload, $log, $timeout, $interval, $parse, $rootScope, uiError) {
    return {
        ...
        link: function (scope, elem, attrs) {
            ...
            /* Theses are persisted until the control is re-initialized in order to to keep data
             * between the control and the application logic consistent - i.e. to avoid condition where these are reset
             * immediately after an new upload has just incremented them.
             */
            var sequenceNum = 0;
            scope.totalFilesToUpload = 0;
            ...
            scope.onFileSelect = function ($files, $event) {
                ...
                scope.uploadResult = [];
                scope.selectedFiles = scope.filterValidFiles($files);
             ...
             */
            scope.filterValidFiles = function (items) {
                ...
                if (fileTypeRegexp) {
                    ...
                    for (var i = 0; i < items.length; i++) {
                        // add check to see that file is not empty
                        if (items[i].size <= 0 ){
                            scope.invalidFilesList.push(items[i]);
                        } else if (items[i].type.length > 0) {
                            ...
                            // No file extensions supplied, and no file type in the associated file.  Add it to the list
                            // of invalid files.
                            scope.invalidFilesList.push(items[i]);
                        }
                    }
            ...
            scope.doUpload = function (sequenceNum, thumbnailImage) {
                ...
                $upload.upload({url: url, file: filesToUpload})
                    ...
                    .error(function (response) {
                        removeFromUploadQueue(sequenceNum);
                        if (scope.failure) {
            ...
            /**
             * Clears the upload queue for the files specified.
             * Takes a lists of the sequences numbers of files to removed.
             */
            scope.$on('clear-file-upload-progress', function (event, aSequenceNums) {
                _.forEach(aSequenceNums, function (sequenceNum) {
                    delete scope.uploadQueue[sequenceNum];
                });
            });
            ...
            if (!scope.dropzoneId) {
                scope.dropzone = $compile('<div ' + acceptAttribute + ' id="body_dropzone" class="ui-dropzone" ng-multiple="' + scope.multiple + '" ng-file-drop ng-file-change="onFileSelect($files, $event)" drag-over-class="file-upload-over-class" ng-class="{overdropzone: overdropzone.display, validdropzone: overdropzone.valid}"></div>');
                ...
                angular.element(body).on('dragover', function (evt) {
                    ...
                    if (evt.dataTransfer.items.length > 0) {
                        if (evt.dataTransfer.items[0].kind === 'file') {
                            if (scope.filterValidFiles(evt.dataTransfer.items).length > 0) {
                ...
                if (dropzone) {
                    ...
                    dropzone.setAttribute('ng-multiple', scope.multiple);
            ...
            if (scope.uploadButtonId) {
                ...
                uploadButtonArray.forEach(function (uploadButtonId) {
                    ...
                    if (uploadButton) {
                        angular.element(uploadButton).attr('ng-file-select', '').attr('ng-file-change', 'onFileSelect($files, $event)');
                        if (scope.multiple) {
                            angular.element(uploadButton).attr('multiple', 'multiple');
                        }
            ...
            scope.cancelUpload = function (file) {
                ...
                removeFromUploadQueue(file.sequenceNum);
            };


            /**
             * function to remove an upload from the upload queue and ensure that appropriate variables are adjusted.
             * @param sequenceNum the sequence number of the file to be removed.
             */
            var removeFromUploadQueue = function (sequenceNum) {
                delete scope.uploadQueue[sequenceNum];
        ...
        controller: ['$scope', function ($scope) {
            ...
            // Returns the uploadQueue array.
            this.getUploadQueue = function () {
                return $scope.uploadQueue;
            };
        }]
    };
};
